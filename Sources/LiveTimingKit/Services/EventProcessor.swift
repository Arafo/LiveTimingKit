...
public actor LiveTimingDefaultEventProcessor: LiveTimingEventProcessor {
    ...
    public func process(event: RawEvent) async throws {
        ...
        case .timingData:
            let timingData = try event.payload.decode(TimingData.self, encoder: encoder, decoder: decoder)
            apply(.timingData(timingData))

        case .heartbeat:
            let heartbeat = try event.payload.decode(Heartbeat.self, encoder: encoder, decoder: decoder)
            apply(.heartbeat(heartbeat))
            
        case .timingAppData:
            let timingAppData = try event.payload.decode(TimingAppData.self, encoder: encoder, decoder: decoder)
            apply(.timingAppData(timingAppData))
            
        case .driverList:
            let driverList = try event.payload.decode(DriverList.self, encoder: encoder, decoder: decoder)
            apply(.driverList(driverList))

        case .carData:
            let cardDate = try event.payload.decode(CarData.self, encoder: encoder, decoder: decoder)
        ...
        case .position:
            let positionData = try event.payload.decode(PositionData.self, encoder: encoder, decoder: decoder)
            apply(.position(positionData))
            
        case .positionZ:
            //let data = try event.payload.decode(String.self)
            //let data = try Data(base64Encoded: event.payload.decode(String.self))
            ...
            let positionZData = try event.payload.decode(PositionZ.self, encoder: encoder, decoder: decoder)
            apply(.positionZ(positionZData))

        case .weatherData:
            let weatherData = try event.payload.decode(WeatherData.self, encoder: encoder, decoder: decoder)
            apply(.weather(weatherData))
            
        case .timingStats:
            let timingStats = try event.payload.decode(TimingStats.self, encoder: encoder, decoder: decoder)
            apply(.timingStats(timingStats))
            
        case .lapCount:
            let lapCount = try event.payload.decode(LapCount.self, encoder: encoder, decoder: decoder)
            apply(.lapCount(lapCount))
            
        case .sessionInfo:
            let sessionInfo = try event.payload.decode(SessionInfo.self, encoder: encoder, decoder: decoder)
            apply(.sessionInfo(sessionInfo))

        case .raceControlMessages:
            let messages = try event.payload.decode(RaceControlMessages.self, encoder: encoder, decoder: decoder)
            apply(.raceControlMessages(messages))

        case .teamRadio:
            let teamRadio = try event.payload.decode(TeamRadio.self, encoder: encoder, decoder: decoder)
            apply(.teamRadio(teamRadio))

        case .tyreStintSeries:
            let series = try event.payload.decode(TyreStintSeries.self, encoder: encoder, decoder: decoder)
            apply(.tyreStintSeries(series))

        case .trackStatus:
            let status = try event.payload.decode(TrackStatus.self, encoder: encoder, decoder: decoder)
            apply(.trackStatus(status))

        case .topThree:
            let topThree = try event.payload.decode(TopThree.self, encoder: encoder, decoder: decoder)
            apply(.topThree(topThree))

        case .sessionData:
            let sessionData = try event.payload.decode(SessionData.self, encoder: encoder, decoder: decoder)
            apply(.sessionData(sessionData))

        case .extrapolatedClock:
            let clock = try event.payload.decode(ExtrapolatedClock.self, encoder: encoder, decoder: decoder)
            apply(.extrapolatedClock(clock))

        case .championshipPrediction:
            let prediction = try event.payload.decode(ChampionshipPrediction.self, encoder: encoder, decoder: decoder)
            apply(.championshipPrediction(prediction))

        case .pitStopSeries:
            let series = try event.payload.decode(PitStopSeries.self, encoder: encoder, decoder: decoder)
            apply(.pitStopSeries(series))

        case .pitLaneTimeCollection:
            let collection = try event.payload.decode(PitLaneTimeCollection.self, encoder: encoder, decoder: decoder)
