        ...
        AsyncStream { continuation in
            ...
            Task { [connection] in
                await connection.on("feed") { @Sendable (id: String, data: [String: AnyCodable], time: String) in
                    ...
                    case .heartbeat:
                         let heartbeat = try! data.to(Heartbeat.self)
                            await self.apply(.heartbeat(heartbeat))
                        
                        
                    case .timingAppData:
                         let timingAppData = try! data.to(TimingAppData.self)
                            await self.apply(.timingAppData(timingAppData))
                        
                    default:
                        print("*** Topoc Id not parsed: \(id)")
                        break
                    }
                }
